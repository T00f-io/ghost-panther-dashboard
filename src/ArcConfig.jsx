...
export default function ArcConfig({ onUpdate }) {
  ...
  async function handleSave() {
    ...
    // Deactivate all existing arcs
    await supabase
      .from('arc_config')
      .update({ active: false })
      .eq('active', true)

    // Insert new active arc
    const { error } = await supabase
      .from('arc_config')
      .insert({ arc_name: arcName, start_date: startDate, duration_days: parseInt(duration), active: true })
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Gohan: Controlled Power"
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 py-2 px-4 rounded-lg bg-white text-zinc-950 text-sm font-medium hover:bg-zinc-200 transition-colors disabled:opacity-50"
            >
              {saving ? 'Saving...' : 'Start Arc'}
            </button>
            <button
